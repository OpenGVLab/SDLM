...
class ModelArguments:
    ...
    add_visionllm_tokens: bool = field(
        default=False,
        metadata={'help': 'Set to True to add some special tokens of VisionLLM.'}
    )
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        data_dir = data_dir.replace('metas2/', 'metas2/cache/')
    ...
    def multi_modal_get_item(self, data_item):
        ...
        start_idx = 1 if data_item['conversations'][0]['from'] == 'system' else 0
        if '<image>' not in data_item['conversations'][start_idx]['value']:
            data_item['conversations'][start_idx]['value'] = '<image>\n' + data_item['conversations'][start_idx]['value']
    ...
    def video_get_item(self, data_item):
        ...
        start_idx = 1 if data_item['conversations'][0]['from'] == 'system' else 0
        if '<video>' not in data_item['conversations'][start_idx]['value']:
            data_item['conversations'][start_idx]['value'] = '<video>\n' + data_item['conversations'][start_idx]['value']
        ...
        data_item['conversations'][start_idx]['value'] = data_item['conversations'][start_idx]['value'].replace(
            '<video>\n', special_tokens + '\n')
...
def main():
    ...
    if model_args.add_visionllm_tokens:
        visionllm_token_list = [
            # tool calling
            '<|agent_tag|>', '<function=', '</function>', '<|image|>', '<|image_list|>',
            '<|video|>', '<|video_list|>', '<|audio|>', '<|audio_list|>',
            # placeholder
            '<region>', '<reg>', '</reg>', '<det>', '</det>', '<grd>', '</grd>', '<gen>',
            '</gen>', '[DET]', '[GRD]', '[SEG]', '[OBJ]', '[POSE]', '[GEN]', '[EDIT]',
            '[EMB]', '[EMB2]', '[EMB3]', '[EMB4]', '[EMB5]', '[EMB6]', '[EMB7]', '[EMB8]'
        ]
        num_new_tokens += tokenizer.add_tokens(visionllm_token_list, special_tokens=True)
