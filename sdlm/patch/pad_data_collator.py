...
def concat_pad_data_collator(features, max_item_length=None, pad_id=0):
    ...
    # print(f'{max_item_length=} {pad_id=}')
    for idx in range(len(features)):
        feat = features[idx]
        temp_input_ids = torch.LongTensor([pad_id] * max_item_length)
        temp_input_ids[:feat['input_ids'].shape[0]] = feat['input_ids']
...
def count_leading_negatives(labels):
...
def concat_pad_data_collator_pure_text(features, max_item_length=None, pad_id=0):
    ...
    for idx in range(len(features)):
        ...
        temp_attention_mask = torch.LongTensor([False] * max_item_length)
    ...
    #     print(f'concat_pad_data_collator_pure_text {i=} item')
